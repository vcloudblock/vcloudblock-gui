...
// The tests below require VCloudBlock Link to be unavailable, so we can trigger
...
describe('Hardware extension connection modal', () => {
    ...
    test('Message saying VCloudBlock Link is unavailable (BLE)', async () => {
        await driver.quit();
        driver = getDriver();
        ...
        findByText('VCloudBlock Link'); // VCloudBlock Link is mentioned in the error modal

        const logs = await getLogs();
        await expect(logs).toEqual([]);
    });

    test('Message saying VCloudBlock Link is unavailable (BT)', async () => {
        await loadUri(uri);
